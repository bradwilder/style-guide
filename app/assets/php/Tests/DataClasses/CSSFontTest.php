...
require_once __ASSETS_PATH . '/php/DataClasses/CSSFont.class.php';

final class CSSFontTest extends TestCase
{
	private $db;
    ...
    public function testConstructorNoID()
    {
		...
		$query = 'update sg_font_type set code = "css" where id = ' . $insertedFontType;
		$this->db->query($query);
		
		$new = new CSSFont($this->db);
	...
	public function testConstructorIDMatch()
    {
		...
		$new = new CSSFont($this->db, $insertedFont);
	...
	public function testRead()
    {
		...
		$query = 'update ' . self::$tableName . ' set directory = "pineapple", cssFile = "haystack" where baseID = ' . $insertedFont;
		$this->db->query($query);
		...
		$new->read();
		$this->assertEquals('pineapple', $new->directory);
		$this->assertEquals('haystack', $new->cssFile);
	}
	...
	public function testWrite()
    {
		...
		$new->directory = 'pineapple';
		$new->cssFile = 'haystack';
		$new->write();
		$row = $this->getByID($insertedFont);
		$this->assertEquals('pineapple', $row['directory']);
		$this->assertEquals('haystack', $row['cssFile']);
	}
	...
	public function testWriteNull()
	{
		...
		$query = 'update ' . self::$tableName . ' set directory = "pineapple" where baseID = ' . $insertedFont;
		$this->db->query($query);
