<tr class="<?=($userListItem->user->id == $currentUser->id ? 'type__label--emphasis' : '')?>">
	...
	<td class="users-table__sessions">
		<?php if ($userListItem->user->sessions > 0) { ?>
			<a class="type__link" data-modal="#sessionsModal"><?=$userListItem->user->sessions?></a>
		<?php } else { echo 0; } ?>
	</td>
	<td class="users-table__requests">
		<?php if (count($userListItem->user->requests) > 0) { ?>
			<a class="type__link" data-modal="#requestsModal"><?=count($userListItem->user->requests)?></a>
		<?php } else { echo 0; } ?>
	</td>
	...
	<td>
		<i class="tables__edit-button fa fa-pencil" data-modal="#editUserModal"></i>
		<?php
			...
			{
				echo '<i class="tables__edit-button activate-request fa fa-envelope"></i>';
			}
		?>
		<?php if ($userListItem->user->id != 1) { ?>
			<?php if (!$userListItem->user->isDeleted) { ?>
				<i class="tables__edit-button fa fa-trash" data-modal="#deleteUserModal"></i>
			<?php } else { ?>
